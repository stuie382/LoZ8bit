...
public class BlueDarknut extends Monster {
    ...
    public BlueDarknut(int theX, int theY, ZeldaPlayWindow theWindow, int theNum) {

        super(theX, theY, theWindow, theNum);

        for (int i = 0; i < 8; i++) {
            try {
                this.monPic[i] = ImageIO.read(new File("src/main/resources/sprites/monster/Darknut/B" + i + ".gif"));
                //Saves each of the pictures for easy access
            } catch (IOException e) {
            }
        }
