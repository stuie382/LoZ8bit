...
public class BlueOctorok extends Monster {
    ...
    public BlueOctorok(int theX, int theY, ZeldaPlayWindow theWindow, int theNum) {

        super(theX, theY, theWindow, theNum);

        for (int i = 0; i < 8; i++) {
            try {
                this.monPic[i] = ImageIO.read(new File("src/main/resources/sprites/monster/Octorok/B" + i + ".gif"));
            } catch (IOException e) {
            }
        }
